...
async function selectTitles() {
    ...
    try{
        let values = await global.conn.collection(login.collection_name).find({published: true}, {projection: {Titulo: true, likes: true, Escolhas: true, owner_name: true}}).sort({likes: -1}).limit(6).toArray()
        return values
    }
    catch(err){
...
async function loadHistory(id) {
    ...
    try{
        id = ObjectId(id)
        let value = await global.conn.collection(login.collection_name).findOne({_id: id})
        return value
    }catch(err){
...
async function like(id, qlikes){
    if (!done){
        return false
    }
    try{
        id = ObjectId(id)
        let value = await global.conn.collection(login.collection_name).updateOne({_id: id},{ $inc: { likes: qlikes }});
