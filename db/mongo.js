...
async function selectTitles() {
    ...
    try{
        let values = await global.conn.collection(login.collection_name).find({published: true}, {projection: {Titulo: true, likes: true, Escolhas: true}}).sort({likes: -1}).limit(6).toArray()
        return values
    }
    catch(err){
